...
export function HomeCalculator(){
  ...
  return <main className="research-lab">
    <header className="hero lab-hero" id="top"><SiteHeader/><div className="hero-copy lab-hero-copy"><p className="eyebrow">운영 중 매장 인수 판단 도구</p><h1>무인아이스크림 인수 계산기</h1><p className="hero-lead">감으로 인수하지 않도록,<br/><strong>매출·비용·권리금과 운영조건을 숫자로 확인합니다.</strong></p><p>운영 중인 무인아이스크림 할인점의 조건을 입력하면 예상 월순이익, 권리금 회수기간과 인수 적합도를 확인할 수 있습니다.</p><div className="hero-actions"><a className="hero-primary" href="#diagnosis" aria-label="무료로 인수 조건 계산하기">무료로 인수 조건 계산하기</a><a className="hero-secondary" href={siteConfig.checklistUrl} target="_blank" rel="noopener noreferrer" aria-label="무료 인수 체크리스트 열기, 새 탭에서 열림">인수 체크리스트 다운로드</a></div><div className="hero-points"><span>✓ 개인정보 수집 없음</span><span>✓ 계산 데이터 서버 저장 없음</span><span>✓ 무료 인수 진단</span></div><DiagnosisCount/></div><div className="lab-manifest"><span>ICE LAB · ACQUISITION 001</span><p>계산 결과는 참고용 추정치입니다.<br/>실제 매물자료와 함께 보수적으로 검토하세요.</p></div></header>
    <TrustSection/>
    <section className="content-shell lab-shell"><AudienceSection/><section className="lab-intro"><div><span>ACQUISITION METHOD</span><h2>인수 조건을 입력하고<br/>핵심 지표를 한 번에</h2></div><div className="lab-steps"><p><b>01</b> 확인 가능한 매출과 비용 조건을 입력합니다.</p><p><b>02</b> 예상 순수익과 권리금 회수기간을 계산합니다.</p><p><b>03</b> 입력값과 결과는 서버에 저장되지 않으며 현재 화면에서만 확인합니다.</p></div></section>
      ...
      {result&&<section className="results" id="result"><div className="section-heading"><span>02</span><div><h2>무료 인수 진단 결과</h2><p>입력한 조건으로 계산한 참고용 결과입니다.</p></div></div><ResultCard result={result}/><div className="result-action-grid"><article className="conversion-banner recalculate-conversion"><div><span>STEP 01 · 조건 비교</span><h3>다시 계산하기</h3><p>매출이나 비용 조건을 바꿔 보수적으로 다시 계산해보세요.</p></div><button className="secondary-button" aria-label="인수 조건 다시 계산하기" onClick={()=>{setResult(null);document.querySelector("#diagnosis")?.scrollIntoView({behavior:"smooth",block:"start"})}}>다시 계산하기</button></article><EbookCTA buttonLabel="전자책 보기"/><ReportCTA/></div><p className="storage-notice">입력값과 계산 결과는 서버에 저장되지 않으며 현재 화면에서만 확인할 수 있습니다.</p><p className="disclaimer">본 진단은 참고용 추정치이며 실제 수익이나 투자 결과를 보장하지 않습니다.</p></section>}
